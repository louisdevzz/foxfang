#!/bin/bash
# Setup FoxFang Auth Management System
...
echo "=== FoxFang Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'foxfang-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  FoxFang message: Send warning via FoxFang itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/foxfang-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/foxfang-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/foxfang-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now foxfang-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status foxfang-auth-monitor.timer"
echo ""
echo "Quick commands:"
