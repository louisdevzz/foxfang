...
async function main() {
  ...
  const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "foxfang-zai-fallback-"));
  const stateDir = path.join(baseDir, "state");
  const configPath = path.join(baseDir, "foxfang.json");
  ...
  const sessionId = process.env.FOXFANG_ZAI_FALLBACK_SESSION_ID ?? randomUUID();
  ...
  const run1 = await runCommand(
    "run1",
    ["foxfang", "agent", "--local", "--session-id", sessionId, "--message", toolPrompt],
    envValidAnthropic,
  );
  if (run1.code !== 0) {
  ...
  const run2 = await runCommand(
    "run2",
    ["foxfang", "agent", "--local", "--session-id", sessionId, "--message", followupPrompt],
    envInvalidAnthropic,
  );
