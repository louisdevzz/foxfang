...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      ...
      "",
      "FoxFang is a hobby project and still in beta. Expect sharp edges.",
      "By default, FoxFang is a personal agent: one trusted operator boundary.",
      "This bot can read files and run actions if tools are enabled.",
      "A bad prompt can trick it into doing unsafe things.",
      "",
      "FoxFang is not a hostile multi-tenant boundary by default.",
      "If multiple users can message one tool-enabled agent, they share that delegated tool authority.",
      "",
      "If you’re not comfortable with security hardening and access control, don’t run FoxFang.",
      ...
      "",
      "Must read: https://docs.foxfang.ai/gateway/security",
    ].join("\n"),
    "Security",
  );
  ...
  if (compatibilityNotices.length > 0) {
    await prompter.note(
      [
        ...
        `Inspect: ${formatCliCommand("foxfang plugins inspect --all")}`,
      ].join("\n"),
