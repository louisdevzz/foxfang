...
FoxFang 会从 `~/.foxfang/foxfang.json` 读取可选的 <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> 配置。

如果该文件缺失，FoxFang 会使用安全的默认值。添加配置的常见原因包括：
...
<Tip>
**刚接触配置？** 从 `foxfang onboard` 开始进行交互式设置，或者查看 [Configuration Examples](/gateway/configuration-examples) 指南，获取完整的可复制粘贴配置。
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    foxfang onboard       # 完整设置向导
    foxfang configure     # 配置向导
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    直接编辑 `~/.foxfang/foxfang.json`。Gateway 网关会监视该文件并自动应用更改（参见[热重载](#config-hot-reload)）。
  </Tab>
</Tabs>
...
<Warning>
FoxFang 只接受完全符合 schema 的配置。未知键、类型格式错误或无效值都会导致 Gateway 网关**拒绝启动**。唯一的根级例外是 `$schema`（字符串），这样编辑器就可以附加 JSON Schema 元数据。
</Warning>

当校验失败时：

- Gateway 网关不会启动
- 只有诊断命令可用（`foxfang doctor`、`foxfang logs`、`foxfang health`、`foxfang status`）
- 运行 `foxfang doctor` 以查看具体问题
- 运行 `foxfang doctor --fix`（或 `--yes`）以应用修复
...
<AccordionGroup>
  ...
  <Accordion title="为官方 iOS 构建启用基于 relay 的推送">
    基于 relay 的推送在 `foxfang.json` 中配置。
    ...
    - `FOXFANG_APNS_RELAY_BASE_URL` 和 `FOXFANG_APNS_RELAY_TIMEOUT_MS` 仍可作为临时环境变量覆盖使用。
    - `FOXFANG_APNS_RELAY_ALLOW_HTTP=true` 仍是仅限 loopback 的开发逃生口；不要在配置中持久化 HTTP relay URL。
...
Gateway 网关会监视 `~/.foxfang/foxfang.json` 并自动应用更改 —— 对于大多数设置，无需手动重启。
...
<AccordionGroup>
  <Accordion title="config.apply（完整替换）">
    ...
    <Warning>
    `config.apply` 会替换**整个配置**。部分更新请使用 `config.patch`，单个键请使用 `foxfang config set`。
    </Warning>
    ...
    ```bash
    foxfang gateway call config.get --params '{}'  # 捕获 payload.hash
...
<Accordion title="Shell 环境变量导入（可选）">
  如果启用，并且预期键名尚未设置，FoxFang 会运行你的登录 shell，并且只导入缺失的键：
...
环境变量等价项：`FOXFANG_LOAD_SHELL_ENV=1`
</Accordion>
