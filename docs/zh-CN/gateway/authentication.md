...
FoxFang 支持模型提供商使用 OAuth 和 API key。对于始终在线的 Gateway 网关
...
2. 将它放在 **Gateway 网关主机** 上（运行 `foxfang gateway` 的机器）。
...
- 优先级顺序：
  - `FOXFANG_LIVE_<PROVIDER>_KEY`（单个覆盖值）
  - `<PROVIDER>_API_KEYS`
  - `<PROVIDER>_API_KEY`
  - `<PROVIDER>_API_KEY_*`
- Google 提供商还将 `GOOGLE_API_KEY` 作为额外回退项。
- 使用前会对同一组 key 列表去重。
- 仅当出现限流错误时，FoxFang 才会使用下一个 key 重试（例如
...
运行 `foxfang models status` 以确认哪个配置文件即将过期。如果该配置文件
