...
summary: 在一台主机上运行多个 FoxFang Gateway 网关（隔离、端口和配置档案）
title: 多个 Gateway 网关
x-i18n:
...
- `FOXFANG_CONFIG_PATH` — 每个实例单独的配置文件
- `FOXFANG_STATE_DIR` — 每个实例单独的会话、凭证、缓存
...
配置档案会自动限定 `FOXFANG_STATE_DIR` + `FOXFANG_CONFIG_PATH`，并为服务名称添加后缀。
