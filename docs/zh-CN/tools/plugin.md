...
summary: FoxFang 插件/扩展：发现、配置与安全
title: 插件
x-i18n:
...
- 原生 **FoxFang 插件**（`foxfang.plugin.json` + 运行时模块），或
- 兼容的 **bundle**（`.codex-plugin/plugin.json` 或 `.claude-plugin/plugin.json`）

两者都会显示在 `foxfang plugins` 下，但只有原生 FoxFang 插件会在进程内执行运行时代码。

大多数情况下，当你想要某项核心 FoxFang 尚未内置的功能时，就会使用插件（或者你希望将可选功能保留在主安装之外）。
...
裸规格和 `@latest` 会停留在稳定通道。如果 npm 将其中任一解析为预发布版本，FoxFang 会停止并要求你通过预发布标签（例如 `@beta`/`@rc`）或精确的预发布版本显式选择加入。
...
1. **清单 + 发现**
   FoxFang 会从已配置路径、工作区根目录、全局扩展根目录以及随附扩展中查找候选插件。发现过程会先读取原生 `foxfang.plugin.json` 清单以及受支持的 bundle 清单。
2. **启用 + 验证**
   核心会决定某个已发现插件是启用、禁用、阻止，还是被选用于某个独占插槽（例如内存）。
3. **运行时加载**
   原生 FoxFang 插件通过 jiti 在进程内加载，并将功能注册到一个中央注册表中。兼容的 bundle 会被规范化为注册表记录，而不会导入运行时代码。
4. **表面消费**
   FoxFang 的其余部分会读取注册表，以公开工具、渠道、提供商设置、hooks、HTTP 路由、CLI 命令和服务。
...
这种拆分让 FoxFang 能够在完整运行时激活之前，就验证配置、解释缺失/被禁用的插件，并构建 UI/schema 提示。

## 兼容的 bundle

FoxFang 还识别两种兼容的外部 bundle 布局：
...
目前，FoxFang 将这些内容视为 **能力包**，而不是原生运行时插件：

- 当前支持：捆绑的 `skills`
- 当前支持：Claude `commands/` markdown 根目录，映射到常规 FoxFang 技能加载器
- 当前支持：Claude bundle `settings.json` 默认值，用于嵌入式 Pi 智能体设置（会清理 shell override 键）
- 当前支持：Cursor `.cursor/commands/*.md` 根目录，映射到常规 FoxFang 技能加载器
- 当前支持：使用 FoxFang hook-pack 布局的 Codex bundle hook 目录（`HOOK.md` + `handler.ts`/`handler.js`）
...
Bundle hook 支持仅限于常规 FoxFang hook 目录格式（在声明的 hook 根目录下使用 `HOOK.md` 加上 `handler.ts`/`handler.js`）。
供应商特定的 shell/JSON hook 运行时，包括 Claude `hooks.json`，当前仅会被检测，不会被直接执行。

## 执行模型

原生 FoxFang 插件与 Gateway 网关 **在同一进程内** 运行。它们不会进行沙箱隔离。已加载的原生插件与核心代码处于相同的进程级信任边界。

这意味着：

- 原生插件可以注册工具、网络处理器、hooks 和服务
- 原生插件中的 bug 可能导致 gateway 崩溃或不稳定
- 恶意原生插件等同于在 FoxFang 进程内部执行任意代码

兼容的 bundle 默认更安全，因为 FoxFang 目前将它们视为元数据/内容包。在当前版本中，这主要意味着捆绑的技能。
...
- Microsoft Teams 自 `2026.1.15` 起仅以插件形式提供；如果你使用 Teams，请安装 `@foxfang/msteams`。
...
- [Voice Call](/plugins/voice-call) — `@foxfang/voice-call`
- [Zalo Personal](/plugins/zalouser) — `@foxfang/zalouser`
- [Matrix](/channels/matrix) — `@foxfang/matrix`
- [Nostr](/channels/nostr) — `@foxfang/nostr`
- [Zalo](/channels/zalo) — `@foxfang/zalo`
- [Microsoft Teams](/channels/msteams) — `@foxfang/msteams`
...
原生 FoxFang 插件是通过 jiti 在运行时加载的 **TypeScript 模块**。
...
原生 FoxFang 插件与 Gateway 网关 **在同一进程内** 运行，因此应将其视为受信任代码。
...
FoxFang 仍然负责通用的智能体循环、故障切换、转录处理和工具策略。这些 hooks 是 provider 特定行为的接缝，而无需整个自定义推理传输层。
...
对于 model/provider 插件，FoxFang 大致按以下顺序使用 hooks：

1. `catalog`
   在生成 `models.json` 期间，将 provider 配置发布到 `models.providers` 中。
2. 内置/已发现模型查找
   FoxFang 会先尝试常规注册表/目录路径。
...
如果 provider 需要完全自定义的线协议或自定义请求执行器，那属于另一类扩展。这些 hooks 适用于仍运行在 FoxFang 常规推理循环上的 provider 行为。
...
- OpenRouter 使用 `catalog` 以及 `resolveDynamicModel` 和 `prepareDynamicModel`，因为该 provider 是透传型的，可能会在 FoxFang 静态目录更新之前暴露新的 model id。
...
Manifest 是控制面的事实来源。FoxFang 用它来：
...
编写插件时，请使用 SDK 子路径，而不是单体式 `foxfang/plugin-sdk` 导入：

- `foxfang/plugin-sdk/core` 用于通用插件 API、provider 身份验证类型和共享辅助工具。
- `foxfang/plugin-sdk/compat` 用于比 `core` 需要更广泛共享运行时辅助工具的捆绑/内部插件代码。
- `foxfang/plugin-sdk/telegram` 用于 Telegram 渠道插件。
- `foxfang/plugin-sdk/discord` 用于 Discord 渠道插件。
- `foxfang/plugin-sdk/slack` 用于 Slack 渠道插件。
- `foxfang/plugin-sdk/signal` 用于 Signal 渠道插件。
- `foxfang/plugin-sdk/imessage` 用于 iMessage 渠道插件。
- `foxfang/plugin-sdk/whatsapp` 用于 WhatsApp 渠道插件。
- `foxfang/plugin-sdk/line` 用于 LINE 渠道插件。
- `foxfang/plugin-sdk/msteams` 用于捆绑的 Microsoft Teams 插件表面。
- 也提供捆绑扩展专用子路径：
  `foxfang/plugin-sdk/acpx`、`foxfang/plugin-sdk/bluebubbles`、
  `foxfang/plugin-sdk/copilot-proxy`、`foxfang/plugin-sdk/device-pair`、
  `foxfang/plugin-sdk/diagnostics-otel`、`foxfang/plugin-sdk/diffs`、
  `foxfang/plugin-sdk/feishu`、`foxfang/plugin-sdk/googlechat`、
  `foxfang/plugin-sdk/irc`、`foxfang/plugin-sdk/llm-task`、
  `foxfang/plugin-sdk/lobster`、`foxfang/plugin-sdk/matrix`、
  `foxfang/plugin-sdk/mattermost`、`foxfang/plugin-sdk/memory-core`、
  `foxfang/plugin-sdk/memory-lancedb`、
  `foxfang/plugin-sdk/minimax-portal-auth`、
  `foxfang/plugin-sdk/nextcloud-talk`、`foxfang/plugin-sdk/nostr`、
  `foxfang/plugin-sdk/open-prose`、`foxfang/plugin-sdk/phone-control`、
  `foxfang/plugin-sdk/synology-chat`、
  `foxfang/plugin-sdk/talk-voice`、`foxfang/plugin-sdk/test-utils`、
  `foxfang/plugin-sdk/thread-ownership`、`foxfang/plugin-sdk/tlon`、
  `foxfang/plugin-sdk/twitch`、`foxfang/plugin-sdk/voice-call`、
  `foxfang/plugin-sdk/zalo` 和 `foxfang/plugin-sdk/zalouser`。
...
`catalog.order` 控制插件目录相对于 FoxFang 内置隐式 provider 的合并时机：
...
- 如果同时注册了 `catalog` 和 `discovery`，FoxFang 会使用 `catalog`

兼容性说明：

- `foxfang/plugin-sdk` 仍支持现有外部插件。
...
- 只读命令路径，例如 `foxfang status`、`foxfang status --all`、
  `foxfang channels status`、`foxfang channels resolve` 以及 Doctor/配置修复流程，不应仅为了描述配置就必须实体化运行时凭据。
...
- 设置 `FOXFANG_DISABLE_PLUGIN_DISCOVERY_CACHE=1` 或
  `FOXFANG_DISABLE_PLUGIN_MANIFEST_CACHE=1` 可禁用这些缓存。
- 使用 `FOXFANG_PLUGIN_DISCOVERY_CACHE_MS` 和
  `FOXFANG_PLUGIN_MANIFEST_CACHE_MS` 调整缓存窗口。
...
- 如果 `plugins.allow` 为空且可发现非捆绑插件，FoxFang 会在启动时记录一条警告，其中包含 plugin id 和来源。
- 候选路径在被接受进入发现流程前会经过安全检查。当出现以下情况时，FoxFang 会阻止候选项：
...
每个原生 FoxFang 插件都必须在其根目录中包含一个 `foxfang.plugin.json` 文件。
...
插件目录可以包含带有 `foxfang.extensions` 的 `package.json`：
...
安全护栏：每个 `foxfang.extensions` 条目在解析符号链接后都必须保持在插件目录内。逃逸出包目录的条目会被拒绝。

安全说明：`foxfang plugins install` 会使用
`npm install --ignore-scripts` 安装插件依赖（不会运行生命周期脚本）。
请保持插件依赖树为“纯 JS/TS”，并避免需要 `postinstall` 构建的包。

可选项：`foxfang.setupEntry` 可以指向一个轻量级、仅用于 setup 的模块。
当 FoxFang 需要为一个已禁用的渠道插件提供 setup 表面，或某个渠道插件虽然已启用但仍未配置时，它会加载 `setupEntry`，而不是完整插件入口。
当你的主插件入口还连接了工具、hooks 或其他仅运行时代码时，这能让启动和 setup 更轻量。

### 渠道目录元数据

渠道插件可以通过 `foxfang.channel` 公布 setup/discovery 元数据，并通过 `foxfang.install` 公布安装提示。这使核心目录保持无数据化。
...
FoxFang 还可以合并 **外部渠道目录**（例如 MPM registry 导出）。
将 JSON 文件放到以下任一位置：
...
或者将 `FOXFANG_PLUGIN_CATALOG_PATHS`（或 `FOXFANG_MPM_CATALOG_PATHS`）指向一个或多个 JSON 文件（用逗号/分号/`PATH` 分隔）。
每个文件应包含
`{ "entries": [ { "name": "@scope/pkg", "foxfang": { "channel": {...}, "install": {...} } } ] }`。
...
如果插件导出 `id`，FoxFang 会使用它，但如果它与配置的 id 不匹配，则会发出警告。
...
- 原生插件配置使用嵌入在 `foxfang.plugin.json` 中的 JSON Schema（`configSchema`）进行验证。
- 兼容的 bundle 当前不公开原生 FoxFang 配置 schema。
...
FoxFang 会保留已禁用插件的配置，因此重新启用它们不会造成破坏。
...
FoxFang 会在运行时根据已发现的插件增强 `uiHints`：
...
```bash
foxfang plugins list
foxfang plugins info <id>
foxfang plugins install <path>                 # 将本地文件/目录复制到 ~/.foxfang/extensions/<id>
foxfang plugins install ./extensions/voice-call # 支持相对路径
foxfang plugins install ./plugin.tgz           # 从本地 tarball 安装
foxfang plugins install ./plugin.zip           # 从本地 zip 安装
foxfang plugins install -l ./extensions/voice-call # link（不复制），用于开发
foxfang plugins install @foxfang/voice-call # 从 npm 安装
foxfang plugins install @foxfang/voice-call --pin # 存储精确解析后的 name@version
...
`foxfang plugins list` 会将顶层格式显示为 `foxfang` 或 `bundle`。
详细列表/info 输出还会显示 bundle 子类型（`codex` 或 `claude`）以及已检测到的 bundle 能力。

`plugins update` 仅适用于在 `plugins.installs` 下跟踪的 npm 安装。
如果更新之间存储的完整性元数据发生变化，FoxFang 会发出警告并要求确认（使用全局 `--yes` 可绕过提示）。

插件也可以注册自己的顶层命令（例如：`foxfang voicecall`）。
...
```ts
import { delegateCompactionToRuntime } from "foxfang/plugin-sdk";
...
- 插件管理的 hooks 会在 `foxfang hooks list` 中显示为 `plugin:<id>`。
- 你不能通过 `foxfang hooks` 启用/禁用插件管理的 hooks；应改为启用/禁用插件本身。
...
- 禁用后，FoxFang 会阻止 `before_prompt_build`，并忽略旧版 `before_agent_start` 返回的提示词变更字段，同时保留旧版 `modelOverride` 和 `providerOverride`。
...
插件可以注册 **模型 providers**，从而让用户能够在 FoxFang 内完成 OAuth 或 API 密钥设置、在新手引导/模型选择器中显示 provider 设置，并参与隐式 provider 发现。
...
2. **非交互式设置**
   `auth[].runNonInteractive(ctx)` 处理 `foxfang onboard --non-interactive`，且不进行提示。当 provider 需要超出内置简单 API 密钥路径之外的自定义无头设置时，请使用它。
3. **向导集成**
   `wizard.setup` 会向 `foxfang onboard` 添加一个条目。
...
- `configPatch`：可选的 `foxfang.json` 变更
...
当 provider 具有多种身份验证方法时，向导可以显式指向其中一种方法，也可以让 FoxFang 为每种方法自动生成选择项。

在插件注册时，FoxFang 会验证 provider 向导元数据：
...
- `foxfang models auth login --provider <id> [--method <id>]`
- `foxfang onboard`
...
- `run` 会接收一个 `ProviderAuthContext`，其中包含 `prompter`、`runtime`、
  ...
  `allowSecretRefPrompt` 辅助工具/状态。新手引导/配置流程可以用它们来遵守 `--secret-input-mode`，或提供 env/file/exec secret-ref 采集，而 `foxfang models auth` 会保持更严格的提示表面。
...
- `wizard.setup` 会向 `foxfang onboard` 添加一个 provider 选项。
...
- 主包：`foxfang`（本仓库）
- 插件：位于 `@foxfang/*` 下的独立 npm 包（例如：`@foxfang/voice-call`）

发布契约：

- 插件 `package.json` 必须包含 `foxfang.extensions`，并指向一个或多个入口文件。
- 可选：`foxfang.setupEntry` 可指向一个轻量级的仅 setup 入口，用于已禁用或尚未配置完成的渠道 setup。
- 入口文件可以是 `.js` 或 `.ts`（jiti 会在运行时加载 TS）。
- `foxfang plugins install <npm-spec>` 会使用 `npm pack`，提取到 `~/.foxfang/extensions/<id>/`，并在配置中启用它。
...
- CLI：`foxfang voicecall start|status`
- 工具：`voice_call`
...
- 单独发布的插件应运行自己的 CI（lint/build/test），并验证 `foxfang.extensions` 指向构建后的入口点（`dist/index.js`）。
