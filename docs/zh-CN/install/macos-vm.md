---
read_when:
  - 你希望 FoxFang 与你的主 macOS 环境隔离
  - 你希望在沙箱中集成 iMessage（BlueBubbles）
  - 你希望拥有一个可重置且可克隆的 macOS 环境
  - 你希望比较本地与托管 macOS VM 选项
summary: 在沙箱化的 macOS VM（本地或托管）中运行 FoxFang，适用于你需要隔离或 iMessage 的场景
title: macOS VM
x-i18n:
...
使用 [Lume](https://cua.ai/docs/lume) 在你现有的 Apple Silicon Mac 上的沙箱化 macOS VM 中运行 FoxFang。
...
VM 会在后台运行。FoxFang 的守护进程会保持 Gateway 网关持续运行。
...
这是在 macOS 上运行的杀手级特性。使用 [BlueBubbles](https://bluebubbles.app) 将 iMessage 添加到 FoxFang。
...
| 未显示 VM IP            | 等待 VM 完全启动后，再次运行 `lume get foxfang`                  |
| 找不到 `lume` 命令      | 将 `~/.local/bin` 添加到你的 PATH                                 |
| 无法扫描 WhatsApp QR 码 | 运行 `foxfang channels login` 时，确保你登录的是 VM 而不是宿主机 |
