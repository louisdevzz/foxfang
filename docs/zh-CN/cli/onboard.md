...
summary: "`foxfang onboard` 的 CLI 参考（交互式新手引导）"
title: onboard
x-i18n:
...
- `--install-daemon` 会先启动受管 Gateway 网关安装路径。不使用它时，你必须已经有一个正在运行的本地 Gateway 网关，例如 `foxfang gateway run`。
...
- 最快开始第一次聊天：`foxfang dashboard`（控制 UI，无需设置渠道）。
