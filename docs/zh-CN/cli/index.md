...
read_when:
  - 添加或修改 CLI 命令或选项时
  - 为新的命令界面编写文档时
summary: "`foxfang` 命令、子命令和选项的 FoxFang CLI 参考"
title: CLI 参考
x-i18n:
...
- `--no-color`：禁用 ANSI 颜色。
- `--update`：`foxfang update` 的简写（仅适用于源码安装）。
- `-V`, `--version`, `-v`：打印版本并退出。
...
注意：插件可以添加额外的顶层命令（例如 `foxfang voicecall`）。

## 安全

- `foxfang security audit` — 审计配置 + 本地状态中常见的安全陷阱。
- `foxfang security audit --deep` — 尽力进行实时 Gateway 网关探测。
- `foxfang security audit --fix` — 收紧安全默认值并对状态 / 配置执行 chmod。

## 密钥

- `foxfang secrets reload` — 重新解析引用，并以原子方式替换运行时快照。
- `foxfang secrets audit` — 扫描明文残留、未解析引用和优先级漂移。
- `foxfang secrets configure` — 用于提供商设置 + SecretRef 映射 + 预检 / 应用的交互式助手。
- `foxfang secrets apply --from <plan.json>` — 应用先前生成的计划（支持 `--dry-run`）。

## 插件

管理扩展及其配置：

- `foxfang plugins list` — 发现插件（机器输出请使用 `--json`）。
- `foxfang plugins info <id>` — 显示插件详情。
- `foxfang plugins install <path|.tgz|npm-spec>` — 安装插件（或将插件路径添加到 `plugins.load.paths`）。
- `foxfang plugins enable <id>` / `disable <id>` — 切换 `plugins.entries.<id>.enabled`。
- `foxfang plugins doctor` — 报告插件加载错误。
...
- `foxfang memory status` — 显示索引统计信息。
- `foxfang memory index` — 重新索引内存文件。
- `foxfang memory search "<query>"`（或 `--query "<query>"`）— 对内存执行语义搜索。
...
非交互式配置助手（get/set/unset/file/validate）。直接运行 `foxfang config` 而不带
子命令会启动向导。
...
- `channels status`：检查 gateway 可达性和渠道健康状态（`--probe` 会运行额外检查；gateway 健康探测请使用 `foxfang health` 或 `foxfang status --deep`）。
- 提示：如果能够检测到常见配置错误，`channels status` 会打印带建议修复方式的警告（随后指向 `foxfang doctor`）。
- `channels logs`：显示 gateway 日志文件中的最近渠道日志。
- `channels add`：未传入任何标志时为向导式设置；传入标志后切换为非交互模式。
  - 当向仍使用单账户顶层配置的渠道添加非默认账户时，FoxFang 会先将账户作用域值移动到 `channels.<channel>.accounts.default`，再写入新账户。
...
绑定规范使用 `channel[:accountId]`。省略 `accountId` 时，FoxFang 可能通过渠道默认值 / 插件 hook 解析账户作用域；否则这就是不带显式账户作用域的渠道绑定。
...
在 OAuth / API 凭据可用时，FoxFang 可以显示提供商用量 / 配额。

展示位置：

- `/status`（可用时添加一行简短的提供商用量信息）
- `foxfang status --usage`（打印完整的提供商明细）
...
- `gateway status` 还能在检测到时显示旧版或额外的 gateway 服务（`--deep` 会增加系统级扫描）。带 profile 名称的 FoxFang 服务会被视为一等公民，不会标记为“额外”。
...
`foxfang models` 是 `models status` 的别名。
...
- `node` 从环境 / 配置解析 gateway 认证（不支持 `--token`/`--password` 标志）：`FOXFANG_GATEWAY_TOKEN` / `FOXFANG_GATEWAY_PASSWORD`，然后是 `gateway.auth.*`。在本地模式下，node host 会有意忽略 `gateway.remote.*`；在 `gateway.mode=remote` 时，`gateway.remote.*` 会根据远程优先级规则参与解析。
...
浏览器控制 CLI（专用 Chrome/Brave/Edge/Chromium）。参见 [`foxfang browser`](/cli/browser) 和 [Browser 工具](/tools/browser)。
