---
read_when:
  - 你想通过 Ollama 在 FoxFang 中运行云端或本地模型
  - 你需要 Ollama 设置和配置指南
summary: 通过 Ollama 运行 FoxFang（云端和本地模型）
title: Ollama
x-i18n:
...
Ollama 是一个本地 LLM 运行时，可以让你轻松在自己的机器上运行开源模型。FoxFang 集成了 Ollama 的原生 API（`/api/chat`），支持流式传输和工具调用，并且在你选择使用 `OLLAMA_API_KEY`（或凭证配置文件）且未定义显式 `models.providers.ollama` 条目时，可以自动发现本地 Ollama 模型。

<Warning>
**远程 Ollama 用户：** 不要在 FoxFang 中使用 `/v1` 的 OpenAI 兼容 URL（`http://host:11434/v1`）。这会破坏工具调用，而且模型可能会把原始工具 JSON 当作纯文本输出。请改用原生 Ollama API URL：`baseUrl: "http://host:11434"`（不要加 `/v1`）。
</Warning>
...
5. 如果你更喜欢手动设置，也可以直接为 FoxFang 启用 Ollama（任意值都可以；Ollama 不需要真实 key）：
...
当你设置 `OLLAMA_API_KEY`（或凭证配置文件），并且**未**定义 `models.providers.ollama` 时，FoxFang 会从 `http://127.0.0.1:11434` 上的本地 Ollama 实例发现模型：

- 查询 `/api/tags`
- 在可用时，尽力通过 `/api/show` 查找 `contextWindow`
- 通过模型名称启发式规则标记 `reasoning`（`r1`、`reasoning`、`think`）
- 将 `maxTokens` 设置为 FoxFang 使用的默认 Ollama 最大 token 上限
- 将所有成本设置为 `0`
...
如果设置了 `OLLAMA_API_KEY`，你可以在提供商条目中省略 `apiKey`，FoxFang 会在可用性检查时自动填充它。
...
FoxFang 默认会将名称中包含 `deepseek-r1`、`reasoning` 或 `think` 的模型视为支持推理的模型：
...
FoxFang 的 Ollama 集成默认使用 **原生 Ollama API**（`/api/chat`），它完全支持同时进行流式传输和工具调用。无需任何特殊配置。
...
当 Ollama 使用 `api: "openai-completions"` 时，FoxFang 默认会注入 `options.num_ctx`，这样 Ollama 就不会静默回退到 4096 上下文窗口。如果你的代理/上游拒绝未知的 `options` 字段，请禁用此行为：
...
对于自动发现的模型，FoxFang 会在 Ollama 提供时使用其报告的上下文窗口，否则回退到 FoxFang 使用的默认 Ollama 上下文窗口。你可以在显式提供商配置中覆盖 `contextWindow` 和 `maxTokens`。
