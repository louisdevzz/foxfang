---
read_when:
  - 你想在 FoxFang 中使用 OpenAI 模型
  - 你想使用 Codex 订阅身份验证而不是 API 密钥
summary: 在 FoxFang 中通过 API 密钥或 Codex 订阅使用 OpenAI
title: OpenAI
x-i18n:
...
OpenAI 明确支持在 FoxFang 这样的外部工具/工作流中使用订阅 OAuth。
...
OpenAI API 用法的模型。FoxFang 会通过 `openai/*` Responses 路径转发这两者。
FoxFang 会有意隐藏过时的 `openai/gpt-5.3-codex-spark` 条目，
因为直接 OpenAI API 调用会在实际流量中拒绝它。

FoxFang **不会**在直接 OpenAI
API 路径上暴露 `openai/gpt-5.3-codex-spark`。`pi-ai` 仍然为该模型提供内置条目，但当前实际 OpenAI API
请求会拒绝它。在 FoxFang 中，Spark 被视为仅限 Codex。
...
OpenAI 当前的 Codex 文档将 `gpt-5.4` 列为当前 Codex 模型。FoxFang
会将其映射为 `openai-codex/gpt-5.4`，用于 ChatGPT/Codex OAuth。

如果你的 Codex 账户有权使用 Codex Spark，FoxFang 也支持：

- `openai-codex/gpt-5.3-codex-spark`

FoxFang 将 Codex Spark 视为仅限 Codex。它不会暴露直接的
`openai/gpt-5.3-codex-spark` API 密钥路径。

当 `pi-ai`
发现 `openai-codex/gpt-5.3-codex-spark` 时，FoxFang 也会保留它。请将其视为依赖 entitlement 且处于实验阶段：Codex Spark 与 GPT-5.4 `/fast` 分开，是否可用取决于已登录的 Codex /
ChatGPT 账户。

### 默认传输

FoxFang 使用 `pi-ai` 进行模型流式传输。对于 `openai/*` 和
...
FoxFang 中，设置 `agents.defaults.models["openai/<model>"].params.serviceTier`，即可
...
启用快速模式后，FoxFang 会应用低延迟 OpenAI 配置：
...
且 `baseUrl` 指向 `api.openai.com`），FoxFang 现在会自动启用 OpenAI 服务端
压缩负载提示：
