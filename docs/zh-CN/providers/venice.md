---
read_when:
  - 你想在 FoxFang 中使用注重隐私的推理
  - 你想获得 Venice AI 设置指引
summary: 在 FoxFang 中使用 Venice AI 注重隐私的模型
title: Venice AI
x-i18n:
...
设置完成后，FoxFang 会显示所有可用的 Venice 模型。请根据你的需要进行选择：
...
当设置了 `VENICE_API_KEY` 时，FoxFang 会自动从 Venice API 发现模型。如果 API 不可访问，则会回退到静态目录。
...
Venice 模型目录会动态更新。运行 `foxfang models list` 以查看当前可用的模型。某些模型可能暂时离线。
