---
read_when:
  - 你想让 FoxFang 连接本地 SGLang 服务器运行
  - 你想通过自己的模型使用兼容 OpenAI 的 `/v1` 端点
summary: 让 FoxFang 与 SGLang 一起运行（兼容 OpenAI 的自托管服务器）
title: SGLang
x-i18n:
...
FoxFang 可以使用 `openai-completions` API 连接到 SGLang。

当你通过 `SGLANG_API_KEY` 选择加入时，FoxFang 还可以**自动发现**
...
定义 `models.providers.sglang` 时，FoxFang 将查询：
