---
read_when:
  - 你想在 FoxFang 中使用 Anthropic 模型
  - 你想使用 setup-token 而不是 API 密钥
summary: 在 FoxFang 中通过 API 密钥或 setup-token 使用 Anthropic Claude
title: Anthropic
x-i18n:
...
在 FoxFang 中，你可以使用 API 密钥或 **setup-token** 进行身份验证。
...
- 当未设置显式 thinking 级别时，Anthropic Claude 4.6 模型在 FoxFang 中默认使用 `adaptive` thinking。
...
FoxFang 的共享 `/fast` 开关也支持直接 Anthropic API 密钥流量。
...
- 这**仅适用于 API 密钥**。Anthropic setup-token / OAuth 身份验证不会遵循 FoxFang 的快速模式层级注入。
- FoxFang 仅对直接发往 `api.anthropic.com` 的请求注入 Anthropic 服务层级。如果你通过代理或网关路由 `anthropic/*`，`/fast` 不会修改 `service_tier`。
...
FoxFang 支持 Anthropic 的 prompt 缓存功能。这**仅适用于 API**；订阅身份验证不会遵循缓存设置。
...
当使用 Anthropic API 密钥身份验证时，FoxFang 会自动对所有 Anthropic 模型应用 `cacheRetention: "short"`（5 分钟缓存）。你可以通过在配置中显式设置 `cacheRetention` 来覆盖此行为。
...
FoxFang 会在 Anthropic API 请求中包含 `extended-cache-ttl-2025-04-11` beta 标志；
如果你覆盖了提供商请求头，请保留它（参见 [/gateway/configuration](/gateway/configuration)）。

## 1M 上下文窗口（Anthropic beta）

Anthropic 的 1M 上下文窗口受 beta 门控。在 FoxFang 中，可通过为受支持的 Opus/Sonnet 模型
...
FoxFang 会将其映射为 Anthropic 请求上的 `anthropic-beta: context-1m-2025-08-07`。
...
注意：Anthropic 当前在使用
OAuth/订阅令牌（`sk-ant-oat-*`）时会拒绝 `context-1m-*` beta 请求。FoxFang 会自动跳过
...
将该令牌粘贴到 FoxFang 中（向导：**Anthropic token（粘贴 setup-token）**），或在 Gateway 网关主机上运行：
...
- 使用 `claude setup-token` 生成 setup-token 并粘贴它，或者在 Gateway 网关主机上运行 `foxfang models auth setup-token`。
...
- 请为该智能体重新运行新手引导，或在
  Gateway 网关主机上粘贴 setup-token / API 密钥，然后使用 `foxfang models status` 验证。

**No credentials found for profile `anthropic:default`**

- 运行 `foxfang models status` 查看当前活动的 auth profile。
- 重新运行新手引导，或为该配置档案粘贴 setup-token / API 密钥。

**No available auth profile (all in cooldown/unavailable)**

- 检查 `foxfang models status --json` 中的 `auth.unusableProfiles`。
