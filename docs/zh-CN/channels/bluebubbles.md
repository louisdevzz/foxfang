...
- FoxFang 通过其 REST API 与其通信（`GET /api/v1/ping`、`POST /message/text`、`POST /chat/:id/*`）。
...
3. 运行 `foxfang onboard` 并选择 BlueBubbles，或手动配置：
...
- 始终设置 webhook 密码。
- 始终要求进行 webhook 身份验证。无论 loopback/代理拓扑如何，除非 BlueBubbles webhook 请求包含与 `channels.bluebubbles.password` 匹配的 password/guid（例如 `?password=<password>` 或 `x-password`），否则 FoxFang 会拒绝该请求。
...
- **输入状态指示**：FoxFang 会发送输入开始事件；BlueBubbles 会在发送后或超时后自动清除输入状态（通过 DELETE 手动停止并不可靠）。
...
FoxFang 可能会显示*短*消息 ID（例如 `1`、`2`）以节省 token。
...
- 直接 handle：`+15555550123`、`user@example.com`
  - 如果某个直接 handle 没有现有私信聊天，FoxFang 会通过 `POST /api/v1/chat/new` 创建一个。这要求启用 BlueBubbles Private API。
...
- 配对码会在一小时后过期；使用 `foxfang pairing list bluebubbles` 和 `foxfang pairing approve bluebubbles <code>`。
...
- FoxFang 会根据 BlueBubbles 服务器的 macOS 版本自动隐藏已知损坏的操作。如果在 macOS 26（Tahoe）上仍显示编辑操作，请手动使用 `channels.bluebubbles.actions.edit=false` 禁用它。
- 状态/健康信息请使用：`foxfang status --all` 或 `foxfang status --deep`。
