---
read_when:
  - 设置 FoxFang 与 Synology Chat
  - 调试 Synology Chat webhook 路由
summary: Synology Chat webhook 设置与 FoxFang 配置
title: Synology Chat
x-i18n:
...
1. 安装并启用 Synology Chat 插件。
   - `foxfang onboard` 现在会在与 `foxfang channels add` 相同的渠道设置列表中显示 Synology Chat。
   - 非交互式设置：`foxfang channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. 在 Synology Chat 集成中：
   - 创建一个入站 webhook 并复制其 URL。
   - 使用你的 secret token 创建一个出站 webhook。
3. 将出站 webhook URL 指向你的 FoxFang Gateway 网关：
   - 默认是 `https://gateway-host/webhook/synology`。
   - 或者使用你自定义的 `channels.synology-chat.webhookPath`。
4. 在 FoxFang 中完成设置。
   - 引导式：`foxfang onboard`
   - 直接设置：`foxfang channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
