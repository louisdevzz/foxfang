...
飞书（Lark）是企业用于消息沟通与协作的团队聊天平台。此插件通过平台的 WebSocket 事件订阅将 FoxFang 连接到飞书/Lark 机器人，因此无需暴露公共 webhook URL 即可接收消息。

---

## 捆绑插件

飞书随当前的 FoxFang 版本一同捆绑提供，因此无需单独安装插件。
...
1. 你已经为飞书运行过 `foxfang channels add`
2. Gateway 网关正在运行（`foxfang gateway status`）
...
2. 运行 `foxfang logs --follow` 并查找 `chat_id`
...
2. 运行 `foxfang logs --follow` 并查找 `open_id`
...
| Command                    | Description                |
| -------------------------- | -------------------------- |
| `foxfang gateway status`  | 显示 Gateway 网关状态      |
| `foxfang gateway install` | 安装/启动 Gateway 网关服务 |
| `foxfang gateway stop`    | 停止 Gateway 网关服务      |
| `foxfang gateway restart` | 重启 Gateway 网关服务      |
| `foxfang logs --follow`   | 跟踪 Gateway 网关日志      |
...
3. 确保已启用**长连接**
4. 确保应用权限完整
5. 确保 Gateway 网关正在运行：`foxfang gateway status`
6. 检查日志：`foxfang logs --follow`
