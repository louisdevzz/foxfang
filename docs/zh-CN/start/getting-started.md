...
read_when:
  - 从零开始进行首次设置
  - 你想用最快的路径开始可用聊天
summary: 在几分钟内安装 FoxFang 并开始你的第一次聊天。
title: 入门指南
x-i18n:
...
<Info>
最快的聊天方式：打开 Control UI（无需设置渠道）。运行 `foxfang dashboard`
并在浏览器中聊天，或在
<Tooltip headline="Gateway host" tip="运行 FoxFang Gateway 网关服务的机器。">网关主机</Tooltip>
...
如果你将 FoxFang 作为服务账户运行，或想使用自定义配置/状态位置：

- `FOXFANG_HOME` 设置用于内部路径解析的主目录。
- `FOXFANG_STATE_DIR` 覆盖状态目录。
- `FOXFANG_CONFIG_PATH` 覆盖配置文件路径。
