...
- CLI 辅助命令：`foxfang onboard`、`foxfang models list`、`foxfang models set <provider/model>`。
...
提供商插件现在可以接管大多数提供商特定逻辑，而 FoxFang 保留
通用推理循环。

典型划分：

- `auth[].run` / `auth[].runNonInteractive`：提供商接管 `foxfang onboard`、`foxfang models auth` 和无头设置的
  新手引导/登录流程
...
以上涵盖了仍适合 FoxFang 常规传输的提供商。若某个提供商
...
- 支持为选定提供商进行通用提供商轮换。
- 通过以下方式配置多个密钥：
  - `FOXFANG_LIVE_<PROVIDER>_KEY`（单个 live 覆盖，最高优先级）
...
- 提供商：`openai`
- 身份验证：`OPENAI_API_KEY`
- 可选轮换：`OPENAI_API_KEYS`、`OPENAI_API_KEY_1`、`OPENAI_API_KEY_2`，以及 `FOXFANG_LIVE_OPENAI_KEY`（单个覆盖）
...
- `openai/gpt-5.3-codex-spark` 在 FoxFang 中被有意抑制，因为 live OpenAI API 会拒绝它；Spark 被视为仅限 Codex
...
- 可选轮换：`ANTHROPIC_API_KEYS`、`ANTHROPIC_API_KEY_1`、`ANTHROPIC_API_KEY_2`，以及 `FOXFANG_LIVE_ANTHROPIC_KEY`（单个覆盖）
- 示例模型：`anthropic/claude-opus-4-6`
- CLI：`foxfang onboard --auth-choice token`（粘贴 setup-token）或 `foxfang models auth paste-token --provider anthropic`
- 直接 API 密钥模型支持共享的 `/fast` 开关和 `params.fastMode`；FoxFang 会将其映射到 Anthropic `service_tier`（`auto` 与 `standard_only`）
...
- CLI：`foxfang onboard --auth-choice openai-codex` 或 `foxfang models auth login --provider openai-codex`
- 默认传输为 `auto`（优先 WebSocket，SSE 回退）
...
- 策略说明：OpenAI Codex OAuth 明确支持 FoxFang 这样的外部工具/工作流。
...
- CLI：`foxfang onboard --auth-choice opencode-zen` 或 `foxfang onboard --auth-choice opencode-go`
...
- 提供商：`google`
- 身份验证：`GEMINI_API_KEY`
- 可选轮换：`GEMINI_API_KEYS`、`GEMINI_API_KEY_1`、`GEMINI_API_KEY_2`、`GOOGLE_API_KEY` 回退，以及 `FOXFANG_LIVE_GEMINI_KEY`（单个覆盖）
- 示例模型：`google/gemini-3.1-pro-preview`、`google/gemini-3-flash-preview`
- 兼容性：使用 `google/gemini-3.1-flash-preview` 的旧版 FoxFang 配置会被标准化为 `google/gemini-3-flash-preview`
...
- 注意：FoxFang 中的 Gemini CLI OAuth 是非官方集成。一些用户报告在使用第三方客户端后遭遇 Google 账号限制。请查看 Google 条款，如果你决定继续，建议使用非关键账号。
- Gemini CLI OAuth 作为内置 `google` 插件的一部分提供。
  - 启用：`foxfang plugins enable google`
  - 登录：`foxfang models auth login --provider google-gemini-cli --set-default`
  - 注意：你**不需要**将客户端 ID 或密钥粘贴到 `foxfang.json` 中。CLI 登录流程会将
...
- Hugging Face Inference 示例模型：`huggingface/deepseek-ai/DeepSeek-R1`；CLI：`foxfang onboard --auth-choice huggingface-api-key`。请参见 [Hugging Face（Inference）](/providers/huggingface)。
...
`foxfang onboard` 和模型选择器中。有关新手引导、云端/本地模式和自定义配置，请参见 [/providers/ollama](/providers/ollama)。
...
- 对于非原生端点上的 `api: "openai-completions"`（任何主机不是 `api.openai.com` 的非空 `baseUrl`），FoxFang 会强制设置 `compat.supportsDeveloperRole: false`，以避免提供商因不支持 `developer` 角色而返回 400 错误。
- 如果 `baseUrl` 为空/省略，FoxFang 会保留默认 OpenAI 行为（即解析为 `api.openai.com`）。
