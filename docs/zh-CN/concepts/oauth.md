---
read_when:
  - 你想端到端了解 FoxFang OAuth
  - 你遇到了令牌失效/登出问题
  - 你想使用 setup-token 或 OAuth 认证流程
  - 你想使用多个账户或配置文件路由
summary: FoxFang 中的 OAuth：令牌交换、存储和多账户模式
title: OAuth
x-i18n:
...
FoxFang 通过 OAuth 支持提供商提供的“订阅认证”，适用于支持此方式的提供商（尤其是 **OpenAI Codex（ChatGPT OAuth）**）。对于 Anthropic 订阅，请使用 **setup-token** 流程。过去有些用户在 Claude Code 之外使用 Anthropic 订阅时曾受限，因此这应视为用户自行选择承担的风险，你应自行核实 Anthropic 当前的政策。OpenAI Codex OAuth 明确支持在 FoxFang 这样的外部工具中使用。本页说明：
...
FoxFang 也支持自带 OAuth 或 API 密钥流程的**提供商插件**。运行方式如下：
...
- 你同时通过 FoxFang _和_ Claude Code / Codex CLI 登录 → 之后其中一个会随机“被登出”

为减少这种情况，FoxFang 将 `auth-profiles.json` 视为一个**令牌汇点**：
...
以上所有位置也都遵循 `$FOXFANG_STATE_DIR`（状态目录覆盖）。完整参考：[/gateway/configuration](/gateway/configuration#auth-storage-oauth--api-keys)
...
在任意机器上运行 `claude setup-token`，然后将其粘贴到 FoxFang 中：
...
FoxFang 的交互式登录流程在 `@mariozechner/pi-ai` 中实现，并接入到各类向导/命令中。
...
向导路径为 `foxfang onboard` → 认证选择 `setup-token`（Anthropic）。

### OpenAI Codex（ChatGPT OAuth）

OpenAI Codex OAuth 明确支持在 Codex CLI 之外使用，包括 FoxFang 工作流。
...
向导路径为 `foxfang onboard` → 认证选择 `openai-codex`。
...
- `foxfang channels list --json`（显示 `auth[]`）
