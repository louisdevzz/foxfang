...
- `agents.defaults.models` 是 FoxFang 可使用的模型允许列表/目录（以及别名）。
...
- 如果你省略提供商，FoxFang 会将输入视为别名或 **默认提供商** 的模型（仅在模型 ID 中没有 `/` 时有效）。
...
`foxfang models`（无子命令）是 `models status` 的快捷方式。
...
`foxfang models scan` 会检查 OpenRouter 的 **免费模型目录**，并且可以
选择性地探测模型对工具和图像的支持。
...
标记持久化以源为准：FoxFang 会根据活动源配置快照（预解析）写入标记，而不是根据运行时已解析的 secret 值写入。
这适用于 FoxFang 重新生成 `models.json` 的所有情况，包括像 `foxfang agent` 这样的命令驱动路径。
