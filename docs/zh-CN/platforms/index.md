...
FoxFang 核心使用 TypeScript 编写。**Node 是推荐的运行时**。
...
- 服务状态：`foxfang gateway status`

## Gateway 网关服务安装（CLI）

使用以下任一方式（均受支持）：

- 向导（推荐）：`foxfang onboard --install-daemon`
- 直接安装：`foxfang gateway install`
- 配置流程：`foxfang configure` → 选择 **Gateway 服务**
- 修复/迁移：`foxfang doctor`（会提供安装或修复服务的选项）

服务目标取决于操作系统：

- macOS：LaunchAgent（`ai.foxfang.gateway` 或 `ai.foxfang.<profile>`；旧版为 `com.foxfang.*`）
- Linux/WSL2：systemd 用户服务（`foxfang-gateway[-<profile>].service`）
