...
你可以在 `~/.foxfang/foxfang.json` 中覆盖此设置：
...
所有日志配置都在 `~/.foxfang/foxfang.json` 的 `logging` 下。
...
- FoxFang 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `foxfang plugins enable diagnostics-otel` 启用插件。
...
- `foxfang.tokens`（计数器，属性：`foxfang.token`、`foxfang.channel`、`foxfang.provider`、`foxfang.model`）
- `foxfang.cost.usd`（计数器，属性：`foxfang.channel`、`foxfang.provider`、`foxfang.model`）
- `foxfang.run.duration_ms`（直方图，属性：`foxfang.channel`、`foxfang.provider`、`foxfang.model`）
- `foxfang.context.tokens`（直方图，属性：`foxfang.context`、`foxfang.channel`、`foxfang.provider`、`foxfang.model`）

消息流：

- `foxfang.webhook.received`（计数器，属性：`foxfang.channel`、`foxfang.webhook`）
- `foxfang.webhook.error`（计数器，属性：`foxfang.channel`、`foxfang.webhook`）
- `foxfang.webhook.duration_ms`（直方图，属性：`foxfang.channel`、`foxfang.webhook`）
- `foxfang.message.queued`（计数器，属性：`foxfang.channel`、`foxfang.source`）
- `foxfang.message.processed`（计数器，属性：`foxfang.channel`、`foxfang.outcome`）
- `foxfang.message.duration_ms`（直方图，属性：`foxfang.channel`、`foxfang.outcome`）

队列 + 会话：

- `foxfang.queue.lane.enqueue`（计数器，属性：`foxfang.lane`）
- `foxfang.queue.lane.dequeue`（计数器，属性：`foxfang.lane`）
- `foxfang.queue.depth`（直方图，属性：`foxfang.lane` 或 `foxfang.channel=heartbeat`）
- `foxfang.queue.wait_ms`（直方图，属性：`foxfang.lane`）
- `foxfang.session.state`（计数器，属性：`foxfang.state`、`foxfang.reason`）
- `foxfang.session.stuck`（计数器，属性：`foxfang.state`）
- `foxfang.session.stuck_age_ms`（直方图，属性：`foxfang.state`）
- `foxfang.run.attempt`（计数器，属性：`foxfang.attempt`）
...
- `foxfang.webhook.error`
  - `foxfang.channel`、`foxfang.webhook`、`foxfang.chatId`、`foxfang.error`
- `foxfang.message.processed`
  - `foxfang.channel`、`foxfang.outcome`、`foxfang.chatId`、`foxfang.messageId`、`foxfang.sessionKey`、`foxfang.sessionId`、`foxfang.reason`
- `foxfang.session.stuck`
  - `foxfang.state`、`foxfang.ageMs`、`foxfang.queueDepth`、`foxfang.sessionKey`、`foxfang.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `foxfang doctor`。
